...
class Import_Facebook_Events_Ical {
	...
	public function import_events( $event_data = array() ){
		...
		if( $event_data['ical_url'] == '' ){
			$ife_errors[] = esc_html__( 'Please provide iCal URL.', 'wp-event-aggregator');
			return;
		}
	...
	protected function get_remote_content( $ical_url ) {
		...
		if ( is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) != 200 ) {
			$ife_errors[] = esc_html__( 'Unable to retrieve content from the provided URL.', 'wp-event-aggregator');
			return false;
		}
